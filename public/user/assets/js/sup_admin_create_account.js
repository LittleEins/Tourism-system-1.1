...
{   
        ...
        $.ajax ({
            ...
            success: function (response) 
            {
                ...
                $.each(response.accounts, function (key, list)
                {
                    ...
                    <td>'+list.location+'</td>\
                    <td class="pr-4 pb-2">\
                    <a href="/supAdmin/edit/pass/account?id='+list.id+'" class="btn btn-primary"><i class="fa fa-edit"></i></a> \
                    </td>\
                    <td class="pr-4 pb-2">\
                    <a href="supAdmin/delete/account?id='+list.id+'" class="btn btn-danger"><i class="fa fa-trash"></i></a> \
                    </td>\
                  </tr>')
                });
    ...
    $(document).on('click','.create_account', function (e)
    {
        ...
        $.ajax ({
            type: "POST",
            url: "/supAdmin/create/staff/account",
            data: data,
            dataType: "json",
            success: function (response) 
            {
