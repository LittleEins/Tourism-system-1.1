...
class staffController extends Controller
{
    ...
    function leave_manual ()
    {
        ...
        $date  = date('F j, Y');
         ...
         else if ($count2 != 0)
         {
            ...
            {
               // checking if destination is already on array
               if (stripos(json_encode($des1),$desti2[$z]->destination) !== false)
               {
                   continue;
               }
               else
               {
                   array_push($des1,$desti2[$z]->destination);
               }
            }
         }
    ...
    function book_data (Request $req)
    {
        ...
        if ($req->group_book != null)
        {
             ...
             if ($req->phone == null)
            {
                ...
                $visit_count = Counting_approves_manual::where('book_number','=', $book_number)->get();
                $visit_group = Counting_group_approve::where('book_number','=', $book_number)->get();
                ...
                for($i = 0; $i < $count; $i++){
                    if (strtolower($req->destination) == strtolower($data2[$i]->name))
                    {
   
                       $map_count = Map_location::where('name','=', strtolower($data2[$i]->name))->first();
                       $count = (int)$map_count->visit_count + $total;
                       $count2 = (int)$map_count->total_visit + $total;
                       
                       $result = DB::table('map_locations')->where('name','=', strtolower($data2[$i]->name))->update(['visit_count'=>$count,'total_visit'=>$count2]);
        ...
        else
        {
            ...
            $time_date  = date('F j, Y g:i:a  ');
             ...
             $data = User::where('id','=', session('LoggedUser'))->first();
             $ap_manual = new Counting_approves_manual;
             $ap_manual->booker_id = $data->id;//
             $ap_manual->staff_id = $data->id;///
             $ap_manual->user_id = $data->id;//
             $ap_manual->first_name = $req->first_nameUser;//
             $ap_manual->last_name = $req->last_nameUser;//
             $ap_manual->phone = $req->phoneUser;//
             $ap_manual->email = $req->emailUser;//
             $ap_manual->gender = $req->genderUser;//
             $ap_manual->address = $req->addressUser;//
             $ap_manual->destination = $req->destination;//
             $ap_manual->book_number = $book_number;//
             $ap_manual->groups = 'solo';
             $ap_manual->book_number = $book_number;//
    ...
    function fetch_checkpoint ()
    {
        ...
        $checkpoint_list = DB::table('book_requests')->where('destination', '=', $acc_data->location )->where('status','=','pending')->get();
    ...
    function manual_entry ()
    {
        ...
        $data['user_data'] = User::where('id','=', session('LoggedUser'))->first();
         ...
         //get all data
        $data['lists'] = Counting_approves_manual::get();

         // split time convert ph time
         $data['tl'] = array();
    ...
    function fetch_group (Request $req)
    {
        $group =  Counting_group_approve::where('book_number',$req->book_number)->get();
    ...
    function fetch_group_delete (Request $req)
    {
        // split word into array
        $parts = explode('/',$req->group_id);


        //user info
        $req_details = Counting_group_approve::where('book_number',$parts[1])->first();
        $total = "1";
        ...
        Counting_group_approve::where('id',$parts[0])->delete();
    ...
    function manual_leave (Request $req)
    {
        $count1 = Counting_approves_manual::where('book_number',$req->id)->count();
        $count2 = Counting_group_approve::where('book_number',$req->id)->count();

        //user info
        $req_details = Counting_approves_manual::where('book_number',$req->id)->first();
        $total = $count1 + $count2;
        ...
        Counting_approves_manual::where('book_number',$req->id)->delete();
        Counting_group_approve::where('book_number',$req->id)->delete();

        return back();
    }

    function manual_update (Request $req)
    {
        date_default_timezone_set('Asia/Manila');
        $datenow = (date("Y/m/d").' '.$req->time);
        $final = strtotime($datenow);

        $details = Counting_approves_manual::where('book_number','=', $req->book_number)->first();
        ...
        if ($details->destination != $req->destination)
        {
            ...
            $count1 = Counting_approves_manual::where('book_number',$req->book_number)->count();
            $count2 = Counting_group_approve::where('book_number',$req->book_number)->count();

            //user info
            $req_details = Counting_group_approve::where('book_number',$req->book_number)->first();
            $total = $count1 + $count2;
            ...
            for($i = 0; $i < $count; $i++){
             ...
             // update new location
             $result = DB::table('counting_approves_manuals')->where('book_number','=', $req->book_number)->update(['destination'=>$req->destination,'time_leave'=>$final]);
             $result = DB::table('counting_group_approves')->where('book_number','=', $req->book_number)->update(['destination'=>$req->destination,'time_leave'=>$final]);
        ...
        {
            $result = DB::table('counting_approves_manuals')->where('book_number','=', $req->book_number)->update(['time_leave'=>$final]);
            $result = DB::table('counting_group_approves')->where('book_number','=', $req->book_number)->update(['time_leave'=>$final]);
    ...
    // system entry list
    function system_entry ()
    {
        $data['user_data'] = User::where('id','=', session('LoggedUser'))->first();

        //get all data
       $data['lists'] = book_request::where('status','approve')->get();
       $data['locations'] = Map_location::where('type','1')->get();
    ...
    function fetch_group_approve (Request $req)
    {
        $group =  Book_data::where('book_number',$req->book_number)->get();
    ...
    function fetch_approve_group_delete (Request $req)
    {
        // split word into array
        $parts = explode('/',$req->group_id);
        ...
        //user info
        $req_details = Book_data::where('book_number',$parts[1])->first();
        $total = "1";
        $update_group_count = Book_request::where('book_number',$parts[1])->first();
        $update_group_count->groups =  (int)$update_group_count->groups - (int)$total;
        $update_group_count->save();
        ...
        book_data::where('id',$parts[0])->delete();
    ...
    function system_leave (Request $req)
    {
        $count1 = Book_request::where('book_number',$req->id)->count();
        $count2 = book_data::where('book_number',$req->id)->count();
        ...
        Book_request::where('book_number',$req->id)->delete();
        Book_data::where('book_number',$req->id)->delete();

        return back();
    }

    function system_update (Request $req)
    {

        $details = Book_request::where('book_number','=', $req->book_number)->first();
        ...
        if ($details->destination != $req->destination)
        {
            ...
            // counting and change the count to other location
            $count1 = Book_request::where('book_number',$req->book_number)->count();
            $count2 = Book_data::where('book_number',$req->book_number)->count();

            //user info
            $req_details = Book_request::where('book_number',$req->book_number)->first();
            $total = $count1 + $count2;
            ...
            for($i = 0; $i < $count; $i++){
             ...
             // update new location
             $result = DB::table('book_requests')->where('book_number','=', $req->book_number)->update(['destination'=>$req->destination]);
             $result = DB::table('book_datas')->where('book_number','=', $req->book_number)->update(['destination'=>$req->destination]);
